...
bool Collision::circleRectCollision(int cx, int cy, int radius, SDL_Rect rect) {
  ...
  // 2 Pixels of grace
  int effectiveRadius = radius - 2;

  bool collision = false;
  if ((dx * dx + dy * dy) <= (effectiveRadius * effectiveRadius)) {
    collision = true; // Circle overlaps rectangle (distance from closest point
                      // <= radius)
  }
  return collision;
}

bool Collision::circleCircleCollision(int cx1, int cy1, int r1, int cx2,
                                      int cy2, int r2) {
  ...
  if ((dx * dx + dy * dy) <= (radiusSum * radiusSum)) {
    collision =
        true; // // Circles overlap (distance between centers <= sum of radii)
  }
  return collision;
}
